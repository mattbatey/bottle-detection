...
# load model
model = YOLO('../weights/best_obb.pt', task="detect")
...
for img_path in image_list:
    ...
    for result in results:
        ...
        j = 0
        for label in labels:
            w = result.obb.xywhr[0][2] # detections.xyxy[j][2] - detections.xyxy[j][0]
            h = result.obb.xywhr[0][3] # detections.xyxy[j][3] - detections.xyxy[j][1]
